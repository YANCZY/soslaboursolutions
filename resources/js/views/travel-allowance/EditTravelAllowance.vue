...
watch(
    ...
    (allowance) => {
        ...
        form.date = allowance.date;
        form.name = allowance.name;
        form.client_id = allowance.client_id;
        form.company_search = allowance.company;
        form.description = allowance.description ?? '';
        form.rate = String(allowance.rate ?? '');
        form.quantity = String(allowance.quantity ?? 1);
        form.amount = String(allowance.amount ?? '0.00');
        form.clearErrors();
        companyError.value = '';
    },
    { immediate: true },
...
const submit = () => {
    ...
    form.put(`/travel-allowance/${props.travelAllowance.id}`, {
        preserveScroll: true,
        onSuccess: () => {
            ...
            <div class="flex max-h-[85vh] min-h-0 flex-col overflow-hidden">
                <DialogHeader class="border-b bg-background px-6 py-4">
                    <div class="flex items-start justify-between gap-3 pr-1">
                        <div class="space-y-1">
                            <DialogTitle>Edit Travel Allowance</DialogTitle>
                            <DialogDescription>
                                Update this travel allowance request.
                            </DialogDescription>
                        </div>
